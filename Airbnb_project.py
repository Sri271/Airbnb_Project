...
if st.button('Dynamic Visualization'):

    db = sqlite3.connect("airbnb_project.db")
    mycursor = db.cursor()

    engine = create_engine('sqlite:///airbnb_project.db', echo=False)
    ...
    with tab8:
        ...
        with col6:
            year=st.selectbox('**select the Year**',
                              (2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012),key='year')
